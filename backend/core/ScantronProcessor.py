...
def find_and_rotate(image_path):
    ...
    image = cv2.imread(image_path) if ".jpg" in image_path else \
            cv2.imread(image_path, cv2.IMREAD_COLOR) if ".png" in image_path else \
            None
    if image is None:
        print(f"Must pass a .png or a .jpg")
        return None
    ...
    if len(vertices) == 4:
        ...
        print(f"There was not 4 vertices!! No perspective transform")
        transformed = four_point_transform(dilated, vertices)
        show_image("2nd-biggest-transformed", transformed)
        return transformed
...
class ScantronProcessor:
    ...
    def save_image(self, save_name, ext:str):
        cv2.imwrite(f"{self.image_path[:-4]}-{save_name}.{ext}", self.image)
    ...
    def process(self):
        ...
        show_image("processing image", self.image)
        ...
        self.save_image("answers-located-graded", os.path.splitext(self.image_path))
